...
k=1;
if measureData(end)-measureData(1)<0; 
    cofec =-1 ; 
 else cofec=1; 
end
...
paramPredi(1,: ) = exp(C).*(delSigma.*sqrt(pi*a)).^m.*dN+a;                                                  % PROBLEM DEFINITION:MODEL DEFINITION

if k<k1
                                                                               %step2. updqte (likehood)
sigl = sqrt(log(1+(paramPredi(end,:)./ paramPredi(1,:)).^2));
mul=log(paramPredi(1,:))-0.5*sigl.^2;

likel = lognpdf(measureData(k),mul,sigl);
                                                                            %step3. resampling
...
if k>k1
sigl = sqrt(log(1+(param(end,:)./param(1,:)).^2));
mul = log(param(1,:))-0.5*sigl.^2; 
eval([ParamResul(1,:) '(k,:) = lognrnd (mul,sigl,1,n);']);
end
end
 %%% POST_PROCESSING
 time = [0:dN:dN*(k-1)]';
